...
export function AmbientCanvas({ className, input, onFallback }: AmbientCanvasProps) {
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(isDarkTheme() ? 0x160f18 : 0xfff7df, 1)
      ...
      new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            ...
            vec3 paper = mix(vec3(1.0, 0.969, 0.875), vec3(0.086, 0.058, 0.094), uTheme) + (grain - 0.5) * mix(0.012, 0.007, uTheme);
            vec3 color = mix(paper, blobColor, merged * 0.98);
            color = mix(color, mix(vec3(0.10, 0.07, 0.28), vec3(0.06, 0.025, 0.07), uTheme), contour * 0.46);
    ...
    const syncTheme = () => {
      ...
      renderer.setClearColor(dark ? 0x160f18 : 0xfff7df, 1)
    }
